...
function getSupabaseConfig() {
  ...
  const supabaseServiceRoleKey = cleanEnvironmentValue(
  ...
  if (!data.user.email) {
    ...
    return {
      error: Response.json(
        {
          success: false,
          error: "Konto użytkownika nie ma przypisanego adresu e-mail.",
        },
        { status: 401 }
      ),
      user: null,
